...
def main():
    ...
    detector_dir_path = f'output/{detector_name}'
    ...
    run_evaluation(
        detector_dir_path, subject_ids, samples, common_pos_test_data,
        slice(3, None), use_sliding_window, detector_class
    )
...
def run_evaluation(
    ...
    pd.DataFrame(stats).to_csv(f'{detector_dir_path}/stats.csv', index=False)
